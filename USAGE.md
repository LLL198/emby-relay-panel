...
管理员和普通用户使用同一个登录页。先打开主站首页登录；登录成功后，管理员首页右上角会出现 **管理后台**，点进去再打开左侧 **节点面板**。普通用户即使手动输入 `/_admin` 也没有权限进入。
...
管理员后台不是另一套密码：使用普通登录页登录后，点击首页右上角的“管理后台”。管理员密码可以在首页“账号安全”修改；修改后所有旧登录会立即失效，需要重新登录。服务器环境变量 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 只用于首次创建管理员，或在你明确修改它们后作为应急重置入口；修改环境变量并重启会让管理员账号密码同步为新值，并注销旧会话。
