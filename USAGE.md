...
普通 VPS 通常可直接使用页面生成的随机高位端口；如果手动填写，公网和内部端口需要一致：

```text
公网 HTTPS 端口：页面默认值，例如 51234
内部 HTTPS 端口：与公网端口相同，例如 51234
```

NAT 示例（无已有 Nginx），服务商端口映射为 `30000 → 51234`：
...
如果节点已经安装并运行 Nginx，面板会保留它的公网监听和现有业务，只在可识别的 Nginx include 目录增加一个带标记的节点代理配置，然后执行 `nginx -t` 和平滑 reload。项目自己的 Nginx 会改为只监听 `127.0.0.1` 的内部高位端口，不会停止、禁用或接管原 Nginx。普通 VPS 会使用检测到的原 Nginx 端口作为公网端口；NAT 节点则使用你填写的公网映射端口，并把它映射到检测到的节点端口。
