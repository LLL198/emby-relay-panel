...
面板统一使用“公网端口 → 内部端口”的方式，不再区分 VPS 或 NAT。页面会默认生成一个随机高位端口：没有端口映射时两个端口保持一致；如果服务商提供了端口映射，只需把外部端口填写在“公网 HTTPS 端口”，内部端口填写节点实际监听端口：
...
如果节点已经安装并运行 Nginx，面板会保留它的公网监听和现有业务，只在可识别的 Nginx include 目录增加一个带标记的节点代理配置，然后执行 `nginx -t` 和平滑 reload。项目自己的 Nginx 会改为只监听 `127.0.0.1` 的内部高位端口，不会停止、禁用或接管原 Nginx。若填写的公网端口与前置 Nginx 实际监听端口不同，则由服务商或云平台负责把公网端口映射到该监听端口。

内部 HTTPS 端口默认随机生成高位端口，也可以手动修改。若填写的端口已被占用，部署会失败并保留原 Nginx 配置；已有 Nginx 但无法识别安全 include 入口时，面板也会拒绝接入。
...
| 公网端口探测失败 | 检查公网端口是否已放行，并映射到页面显示的内部端口。 |
