...
在一台全新的 Debian/Ubuntu、且当前 SSH 会话有 `sudo` 权限的主控机上，可以直接运行：

```bash
curl -fsSL https://raw.githubusercontent.com/LLL198/emby-relay-panel/main/deploy/install.sh | sudo bash
```

脚本会交互询问主站域名、Cloudflare Zone、管理员用户名、证书路径和 Token，然后自动完成：

- 安装 Python、Nginx、SSH、OpenSSL 等依赖
- 下载项目并创建 Python 虚拟环境
- 生成 Fernet 密钥和环境文件（权限 `0600`）
- 安装 `uniproxy.service` 并启动主控面板
- 使用 Cloudflare DNS-01 自动申请主站证书，并生成一个不含 Cloudflare Origin Pull 的基础 Nginx 配置

首次部署不填写管理员密码时，账号是 `admin`，密码是 `123456`，首次登录必须修改。Cloudflare Token 如果留空，脚本会询问已有证书路径并跳过自动申请；添加节点前仍必须按第 6 节补上 Token。自动申请失败时，继续执行第 7 节检查证书和 Nginx。

该脚本目前只支持使用 systemd 的 Debian/Ubuntu；Alpine 请按后文的手动步骤部署。运行远程脚本前请先检查来源，生产环境也可以先下载后审阅：

```bash
curl -fsSL https://raw.githubusercontent.com/LLL198/emby-relay-panel/main/deploy/install.sh -o /tmp/emby-relay-panel-install.sh
less /tmp/emby-relay-panel-install.sh
sudo bash /tmp/emby-relay-panel-install.sh
```
...
# 首次部署可省略 ADMIN_PASSWORD，默认密码为 123456；生产环境建议直接填写随机长密码
...
首次部署如果省略 `ADMIN_PASSWORD`，管理员为 `admin`，默认密码为 `123456`，首次登录后会被要求修改密码。生产环境不要长期使用默认密码。两个 Fernet 密钥不能互换，也不能使用管理员密码代替。丢失密钥可能导致已有邀请码或节点凭据无法解密。
...
第一次成功启动时，程序会用 `ADMIN_USERNAME`/`ADMIN_PASSWORD` 创建管理员账号；如果没有设置 `ADMIN_PASSWORD`，默认登录信息是 `admin` / `123456`，登录后必须先修改密码。之后管理员和普通用户共用 `/login`；管理员登录到用户首页后，会看到进入管理后台的入口。普通用户没有后台权限，即使手动访问 `/_admin` 也会被后端拒绝。
