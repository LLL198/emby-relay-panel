...
3. 管理员先用普通登录页登录，首页右上角会出现“管理后台”；添加节点时只填节点名称、VPS/NAT、IP、SSH 端口、两个 HTTPS 端口和 SSH 密码/私钥。
...
安全默认值：用户源站只允许 HTTPS 且会固定经过验证的公网解析地址；上游 TLS 开启证书校验；面板会使用 `__Host-` 会话 Cookie；后台每个请求都会校验管理员会话角色，普通用户不能靠隐藏链接或直接 URL 调用后台。Cloudflare Token 仅用于主控机集中签发证书，不应复制到节点。
